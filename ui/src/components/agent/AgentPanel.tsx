...
export default function AgentPanel() {
  const { messages, streamState, send, close, loadHistory, draft, setDraft } = useAgent();
  ...
  useEffect(() => {
    ...
    if (lastMsg?.role === 'assistant' && lastMsg.toolName) {
      const richTools = ['apply_to_job', 'get_dossier', 'interview_prep', 'discover_jobs'];
...
function PanelNotifications({ messages }: { messages: AgentMsg[] }) {
  ...
  type BannerItem = { label: string; type: BannerType; href?: string; priority: 0 | 1 | 2 };
      ...
      .then((data) => {
        ...
        const items = (data.events || [])
          ...
          .map((event) => ({
            label: event.title,
            type:
              event.type === 'questions_pending' || event.type === 'application_blocked'
                ? 'question' as const
                : event.type === 'followup_reminder' || event.type === 'ghost_alert' || event.type === 'interview_detected'
                  ? 'alert' as const
                  : 'brief' as const,
            href: event.metadata?.application_id ? `/brief/${String(event.metadata.application_id)}` : undefined,
            priority:
  ...
  for (const msg of messages.slice(-10)) {
    ...
    const action = (r.recommended_next_action && typeof r.recommended_next_action === 'object')
    ...
    if (msg.toolName === 'apply_to_job' && r.status === 'waiting_user_input') {
      const count = Array.isArray(r.pending_questions) ? r.pending_questions.length : 0;
      if (count > 0) {
        pendingItems.push({
  ...
  const mergedItems = [...pendingItems, ...activityItems]
    .filter(
      (item, index, all) =>
        all.findIndex((candidate) => candidate.label === item.label && candidate.href === item.href) === index,
    )
  ...
  return (
    <div style={{
      background: 'var(--el)', border: '1px solid var(--bv)', borderRadius: 8,
      margin: 8, padding: '8px 12px', flexShrink: 0,
    }}>
      <div style={{
            ...
            background:
              item.type === 'question'
                ...
                : item.type === 'alert'
                  ? 'var(--error)'
                  : 'var(--vl)',
            animation: item.priority === 0 ? 'status-pulse 2s infinite' : 'none',
          }} />
          {item.href ? (
            <Link href={item.href} style={{ color: 'var(--vl)', fontSize: 12 }}>{item.label}</Link>
          ) : (
