...
async def list_notifications_endpoint(limit: int = Query(100, ge=1, le=500)):
    await init_db()
    return {"deliveries": await list_notification_deliveries(limit=limit)}


@router.get("/runs/{run_id}/notifications")
async def list_run_notifications_endpoint(run_id: str, limit: int = Query(100, ge=1, le=500)):
    await init_db()
...
async def get_notification_endpoint(delivery_id: str):
    await init_db()
...
async def retry_notification_endpoint(delivery_id: str):
    await init_db()
