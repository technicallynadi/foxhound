...
async def start_research_cascade(
    ...
    """
    task = asyncio.create_task(_run_cascade(user_id, application_id, job_id, match_score))
...
async def _run_cascade(
    ...
    try:
                    ...
                    try:
                        ...
                        async with async_session() as cache_db:
                            cached = TinyFishBriefCache(
                                ...
                                sources_completed=_json.dumps(
                                    ["posting"]
                                    + (["careers"] if careers_data else [])
                                    + (["company"] if company_data else [])
                                ),
            ...
            async with async_session() as check_db:
                ...
                if _current and _current.network_map_json:
                    ...
                    if nm_check.get("contacts") and len(nm_check["contacts"]) > 0:
                        ...
                        logger.info(
                            "Research cascade: network map exists with %d contacts, skipping for %s",
                            len(nm_check["contacts"]),
                            company,
                        )
        except Exception as e:
            ...
            try:
                ...
                if cached_contacts and len(cached_contacts) > 0:
                    ...
                    async with async_session() as net_db:
                        network_result = await network_map(
                            net_db,
                            user_id,
                            {
                        ...
                        try:
                            ...
                            async with async_session() as cache_db:
                                ...
                                if _cached:
                                    ...
                                    await cache_db.commit()
                                    logger.info(
                                        "Research cascade: cached %d contacts for %s", len(contacts_to_cache), company
                                    )
                        except Exception as e:
        ...
        if not has_pathfinder:
            try:
                ...
                if isinstance(cb, dict) and not cb.get("error"):
                    ...
                    if cb.get("hiring_velocity"):
                        parts.append(f"Hiring pace: {cb['hiring_velocity']}")
                    if cb.get("insider_tip"):
                        parts.append(f"Insider tip: {cb['insider_tip']}")
                    if cb.get("tech_stack"):
                        parts.append(f"Tech stack: {', '.join(cb['tech_stack'][:10])}")
        ...
        nm = brief_data.get("network_map", {})
        if isinstance(pf, dict) and not pf.get("error") and isinstance(nm, dict) and nm.get("contacts"):
            high_contacts = [c for c in nm["contacts"] if isinstance(c, dict) and c.get("relevance") == "high"]
